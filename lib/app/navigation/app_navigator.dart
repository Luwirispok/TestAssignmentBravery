...
import 'package:power_bank/domain/enums/bank_cards_screen_type.dart';
...
import '../presentation/screens/vending_machines_on_map/bloc/vending_machines_on_map_bloc.dart';
import '../presentation/screens/vending_machines_on_map/vending_machines_on_map_screen.dart';
...
class AppNavigator {
  static Future<D?> navigate<D>({
    ...
    action.when(
      ...
      navigateToNavigation: (NavigateType type, Settings settings, PhoneNumber phone) {
        navigateType = type;
        route = MaterialPageRoute(
          settings: RouteSettings(name: routeName),
          builder: (BuildContext context) => BlocProvider(
      ...
      navigateToContacts: (NavigateType type) {
        navigateType = type;
        route = MaterialPageRoute(
          settings: RouteSettings(name: routeName),
          builder: (BuildContext context) => BlocProvider(
    ...
    switch (navigateType) {
      ...
      case NavigateType.pushAndRemoveUntil:
        return await Navigator.pushAndRemoveUntil<D?>(
            context, route, (_) => false);
      case NavigateType.popUntil:
        Navigator.popUntil(
            context, (routeForCompare) => route == routeForCompare);
    }
    return null;
  }
}
